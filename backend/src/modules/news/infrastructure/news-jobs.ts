...
export const runSymbolNewsFetchJob = async (): Promise<void> => {
  ...
  const allSymbols = [
    ...new Set([
      ...
      ...portfolios.flatMap((p: any) =>
        p.positions.map((pos: { symbol: string }) => pos.symbol),
      ),
    ]),
  ]
  ...
  logger.info(
    `[NewsCron] Fetching news for ${allSymbols.length} symbols (12s delay between calls)`,
  )
...
const JOB_DEFINITIONS = [
  {
    ...
    handler: runSymbolNewsFetchJob,
    pattern: '*/15 * * * *',
  },
  {
    name: 'news-general-fetch',
    handler: runGeneralNewsFetchJob,
    pattern: '*/30 * * * *',
  },
...
export const startNewsCronJobs = async (): Promise<void> => {
  if (!connection) {
    logger.warn(
      '[NewsCron] No Redis connection found — news fetch jobs will not be registered.',
    )
    return
  }
