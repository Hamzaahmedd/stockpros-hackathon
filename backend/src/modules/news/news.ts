...
export const enrichArticles = async (
  ...
  const [readStates, savedStates, portfolios, watchlistItems] = await Promise.all([
  ...
  const portfolioSymbolSet = new Set(portfolios.flatMap(p => p.positions.map(pos => pos.symbol)))
  const watchlistSymbolSet = new Set(watchlistItems.map(w => w.symbol))

  return articles.map(article => ({
    ...article,
    userContext: {
      inPortfolio: article.relatedSymbols.some(s => portfolioSymbolSet.has(s)),
      inWatchlist: article.relatedSymbols.some(s => watchlistSymbolSet.has(s)),
    },
...
export const rankArticles = (
  ...
  const withRank = articles.map(article => {
    ...
    if (syms.some(s => portfolioSymbols.has(s)))                                        return { ...article, _rank: 1 }
    if (syms.some(s => watchlistSymbols.has(s)))                                        return { ...article, _rank: 2 }
    if (article.category === 'SECTOR' && article.sector && portfolioSectors.has(article.sector)) return { ...article, _rank: 3 }
