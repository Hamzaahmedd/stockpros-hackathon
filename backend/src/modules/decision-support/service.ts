// Consolidated decision-support service
...
import {
    ActionGuidance,
    DecisionResult, EnrichedPortfolioPosition, PortfolioPosition, PortfolioSummary, RawPortfolioRow, RiskLevel,
    VolatilityLevel
} from './types'
...
export const enrichPortfolio = async (
...
): Promise<{ positions: EnrichedPortfolioPosition[]; summary: PortfolioSummary }> => {
  const uniqueSymbols = [...new Set(rows.map((row) => row.symbol.toUpperCase()))]
  ...
  const positions = rows.map((row) => {
    ...
    return {
      ...
      unrealizedPnLPercent: cost > 0 ? Number(((pnl / cost) * 100).toFixed(2)) : 0,
...
export const getUnifiedMarketDecision = async (symbol: string): Promise<UnifiedMarketDecision> => {
  ...
  const actionGuidance = computeActionGuidance(decision.recommendation, rsi, sentiment.trend);
...
export const getHistoricalCloses = async (symbol: string): Promise<number[]> => {
  ...
  try {
    ...
    console.error(`Internal Yahoo Finance Error for ${symbol}:`, error);
  ...
  try {
    ...
    return data.results.map((article) => {
      ...
      return {
        ...
        isEarningsContext: isEarningsSeason &&
          mapPolygonCategory(article.keywords || [], article.title) === 'EARNINGS',
      };
    });
  } catch (error) {
...
export const parseAnalystConsensus = (data: FinnhubRecommendation[]): AnalystConsensus => {
  ...
  const confidencePercent = totalAnalysts > 0 ? Math.round((bullishCount / totalAnalysts) * 100) : 0
...
export const computeSentiment = (feed: NewsSentimentArticle[], targetTicker: string): SentimentSummary => {
  ...
  const recentScore = tickerScores.slice(0, midPoint).reduce((a, b) => a + b, 0) / midPoint
  const olderScore = tickerScores.slice(midPoint).reduce((a, b) => a + b, 0) / (volume - midPoint)

  const change48h = olderScore !== 0
...
const computeAnalystConfidence = (confidencePercent: number, sourceCount: number) => {
...
export const computeDecision = ({
  ...
  const sentimentConfidence = computeSentimentConfidence(sentimentScore, newsVolume)
  const analystConfidence = computeAnalystConfidence(analystConfidencePercent, analystSourceCount)
...
export async function getPortfolioSnapshot(portfolioId: string) {
  ...
  const positionsWithLive = portfolio.positions.map((position) => {
    ...
    return {
      ...
      unrealizedPnLPercent: position.avgEntryPrice > 0
...
export const uploadPortfolio = async (fileType: string | undefined, buffer: Buffer | undefined, userId: string) => {
...
export const getPortfolioDecisionResponse = async (
  ...
  persistDecisionRun(userId, portfolioId, fullResults).catch((err) =>
    console.error('[DecisionPersist] Failed to persist run:', err),
  )

  const positions = decisionMode === 'OVERVIEW'
...
export const calculateFinalConfidence = (opts: {
  ...
  else if (opts.unrealizedPnLPercent < -20) score -= 0.10;
  ...
  return Number(Math.max(0.30, Math.min(0.98, score)).toFixed(2));
}

const getActionGuidance = (currentPrice: number, riskLevel: string, confidence: number): ActionGuidance => {
  const isHighRisk = riskLevel === 'HIGH';
  const holdDuration = isHighRisk
    ? (confidence < 0.6 ? "1–3 days" : "2–10 days")
    : (confidence >= 0.8 ? "5–20 days" : "3–14 days");
  return {
    positionStrategy: {
      add: !isHighRisk,
      hold: true,
...
export const generateBatchDecision = async (portfolioId: string): Promise<DecisionResult[]> => {
  const snapshot = await getPortfolioSnapshot(portfolioId);

  const marketDataPromises = snapshot.positions.map(p => getUnifiedMarketDecision(p.symbol));
  ...
  return snapshot.positions.map((position, index) => {
    ...
    } else if (pnlPct < -5 || (exposure.isOverExposed && exposure.positionPercentOfPortfolio > 25)) {
      portfolioDecision = 'TRIM';
    } else if (exposure.isOverExposed || marketDecision === 'SELL') {
      portfolioDecision = 'HOLD';
    } else if (marketDecision === 'BUY' && exposure.positionPercentOfPortfolio < 20) {
    ...
    const details: string[] = [];
    if (exposure.positionPercentOfPortfolio > 30) details.push("Heavily concentrated position");
    else if (exposure.positionPercentOfPortfolio > 15) details.push("Concentrated position size");
    if (exposure.sectorExposurePercent > 60) details.push("Critical sector over-exposure");
    else if (exposure.sectorExposurePercent > 40) details.push("High sector concentration");
    ...
    if (exposure.positionPercentOfPortfolio < 5) details.push("Small position — low portfolio impact");

    let reasoningSummary: string;
    if (portfolioDecision === 'EXIT') {
      reasoningSummary = "Critical risk detected — position should be closed to protect portfolio health.";
    } else if (portfolioDecision === 'TRIM') {
      reasoningSummary = "Risk metrics indicate reducing this position size to improve portfolio balance.";
    } else if (portfolioDecision === 'ADD') {
      reasoningSummary = "Technicals and portfolio allocation support increasing exposure to this position.";
    } else if (exposure.isOverExposed) {
      reasoningSummary = "Market is favorable, but internal portfolio risk requires holding without adding.";
    } else {
      reasoningSummary = "Technicals and portfolio health are aligned — maintain current position.";
    }
    ...
    return {
      ...
      actionGuidance: getActionGuidance(position.currentPrice, risk.riskLevel, confidence)
    };
  });
};
