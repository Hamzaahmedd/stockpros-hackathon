...
export const startAuthEmailWorker = (): void => {
  const connection = getRedisClient();
  if (!connection) {
    logger.warn('[AuthEmailWorker] No Redis connection found — auth email worker will not be started.');
    return;
  }
  ...
  authEmailWorker = new Worker<AuthEmailJobPayload>(
    ...
    async (job) => {
      ...
      // `from` is intentionally omitted so the transporter default applies,
...
/**
 * Add a magic-link email job to the auth queue.
 * Returns true when the job was successfully enqueued; false when Redis/the
 * queue is unavailable — callers can then fall back to a direct send.
 */
export const enqueueAuthEmail = async (payload: AuthEmailJobPayload): Promise<boolean> => {
  const queue = getAuthEmailQueue();
  if (!queue) {
    logger.warn('[AuthEmailWorker] Redis not connected — magic-link email was not enqueued.');
    return false;
  }
