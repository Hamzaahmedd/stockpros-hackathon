...
export const getStockForecast = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { symbol, period } = validateOrThrow(forecastQueryValidator, req.query);
