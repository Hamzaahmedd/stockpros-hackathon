...
export class FinnhubService extends EventEmitter {
  ...
  private connect() {
    ...
    this.ws.on('open', async () => {
      ...
      // Slower batching (1000ms) is safer for free-tier keys
    ...
    this.ws.on('message', (raw) => {
      try {
        ...
        if (msg.type === 'trade' && Array.isArray(msg.data)) {
          ...
          // Catch rate-limit errors sent inside the socket stream
          logger.warn(
  ...
  private async resubscribeInBatches(
    symbols: string[],
    batchSize = 10,
    delayMs = 1000,
  ): Promise<void> {
  ...
  private reconnectWithBackoff() {
    ...
    // Add random jitter (0-3s) so retries are not perfectly synchronized
    const jitter = Math.random() * 3000
    const delay =
      Math.min(
        60_000,
        baseDelay * Math.pow(2, Math.min(this.reconnectAttempts - 1, 5)),
      ) + jitter

    logger.info(
      `RECONNECTING to Finnhub in ${Math.round(delay)}ms... (Jitter added: ${Math.round(jitter)}ms)`,
    )
