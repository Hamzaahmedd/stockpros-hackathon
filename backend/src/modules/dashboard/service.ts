...
const buildBriefing = async (
  ...
  if (!lastRun) {
    ...
    const buySignals     = results.filter(r => r.portfolioDecision === 'ADD' || r.marketDecision === 'BUY').length
    const holdSignals    = results.filter(r => r.portfolioDecision === 'HOLD').length
    const trimSignals    = results.filter(r => r.portfolioDecision === 'TRIM' || r.portfolioDecision === 'EXIT').length
    const positionsAtRisk = results.filter(r => r.riskLevel === 'HIGH').length

    const headline = [
      buySignals     > 0 ? `${buySignals} position${buySignals > 1 ? 's have' : ' has'} a BUY signal` : null,
      positionsAtRisk > 0 ? `${positionsAtRisk} position${positionsAtRisk > 1 ? 's' : ''} need${positionsAtRisk === 1 ? 's' : ''} attention` : null,
    ].filter(Boolean).join('. ') || 'Your portfolio is stable'

    decisionSupport = {
      available: true,
      reason:    null,
      summary:   { buySignals, holdSignals, trimSignals, positionsAtRisk, lastRunAt: lastRun.runAt },
      headline,
    }
  }
  ...
  const stopLossBreaches = symbolsWithPrice.filter(
    item => item.stopLoss !== null && item.currentPrice !== null && item.currentPrice <= item.stopLoss
  ).length
  ...
  if (overexposedSectors.length > 0)
    portfolioAlertParts.push(`Your portfolio is heavily overexposed to ${overexposedSectors.join(', ')}`)
  if (stopLossBreaches > 0)
    portfolioAlertParts.push(`${stopLossBreaches} stop loss${stopLossBreaches > 1 ? 'es' : ''} breached`)
  if (entryZonesActive > 0)
    portfolioAlertParts.push(`${entryZonesActive} symbol${entryZonesActive > 1 ? 's are' : ' is'} in entry zone`)
  ...
  return { greeting, generatedAt: new Date().toISOString(), decisionSupport, portfolioAlert }
}
...
const computeHealthScore = async (
  userId: string,
  positions: Array<{ symbol: string; sector: string | null; avgEntryPrice: number; quantity: number }>,
  ...
  const maxSectorPct = totalValue > 0
    ...
    : 0
  const diversificationScore = maxSectorPct > 30
  ...
  const watchlistDisciplineScore = watchlistItems.length > 0
  ...
  const label = score >= 80 ? 'Your portfolio is well-structured'
    : score >= 60 ? 'Minor optimizations available'
    : score >= 40 ? 'Some risks need attention'
...
const buildPortfolioSection = async (
  ...
  const priceDataMap = new Map<string, { price: number; changePercent: number }>()
  await Promise.all(
  ...
  positions.forEach(p => {
    ...
    if (changePercent > bestPct)  { bestPct  = changePercent; bestSymbol  = p.symbol }
    if (changePercent < worstPct) { worstPct = changePercent; worstSymbol = p.symbol }
  })

  const totalUnrealizedPnL    = totalValue - totalCost
  const totalUnrealizedPnLPct = totalCost > 0 ? (totalUnrealizedPnL / totalCost) * 100 : 0
  const todayGainLossPct      = totalValue > 0 ? (todayGainLoss / totalValue) * 100 : 0

  const healthScore = await computeHealthScore(userId, positions, lastRun, totalCost)
  ...
  return {
    ...
    bestPerformer:  bestSymbol  ? { symbol: bestSymbol,  changePercent: Number(bestPct.toFixed(2))  } : null,
    worstPerformer: worstSymbol ? { symbol: worstSymbol, changePercent: Number(worstPct.toFixed(2)) } : null,
    healthScore,
  }
}
...
const buildImpactNews = async (userId: string): Promise<{ items: ImpactNewsItem[]; totalCount: number }> => {
  const since = new Date(Date.now() - DASHBOARD_IMPACT_NEWS_HOURS * 60 * 60 * 1000)
  ...
  portfolios.flatMap(p => p.positions).forEach(pos =>
    portfolioSymbolMap.set(pos.symbol, pos.quantity)
  )
  ...
  const items: ImpactNewsItem[] = articles
    .map(article => {
      const matchedPortfolioSymbol = article.relatedSymbols.find(s => portfolioSymbolMap.has(s))
      const matchedWatchlistSymbol = article.relatedSymbols.find(s => watchlistSymbols.has(s))
      const primarySymbol          = matchedPortfolioSymbol ?? matchedWatchlistSymbol ?? article.relatedSymbols[0]
      const sharesHeld             = matchedPortfolioSymbol ? (portfolioSymbolMap.get(matchedPortfolioSymbol) ?? null) : null

      let impact: ImpactNewsItem['impact']
      if (matchedPortfolioSymbol) {
        impact = article.sentiment === 'BEARISH' ? 'NEGATIVE_HOLDING' : 'POSITIVE_HOLDING'
      } else {
        impact = article.sentiment === 'BEARISH' ? 'NEGATIVE_WATCHLIST' : 'POSITIVE_WATCHLIST'
      }
    ...
    .sort((a, b) => {
      const aPriority = a.impact === 'NEGATIVE_HOLDING' || a.impact === 'POSITIVE_HOLDING' ? 0 : 1
      const bPriority = b.impact === 'NEGATIVE_HOLDING' || b.impact === 'POSITIVE_HOLDING' ? 0 : 1
...
const buildSmartTriggers = async (userId: string): Promise<{ items: SmartTrigger[]; totalCount: number }> => {
  const [watchlistItems, portfolios] = await Promise.all([
    ...
    prisma.portfolio.findMany({
      where:   { userId },
      include: { positions: { select: { symbol: true, quantity: true, avgEntryPrice: true } } },
    }),
  ])

  const positionMap = new Map<string, { quantity: number; avgEntryPrice: number }>()
  portfolios.flatMap(p => p.positions).forEach(pos =>
    positionMap.set(pos.symbol, { quantity: pos.quantity, avgEntryPrice: pos.avgEntryPrice })
  )
  ...
  const eventTriggerMap: Record<string, { type: SmartTriggerType; urgency: TriggerUrgency; message: string; action: string }> = {
    EARNINGS_APPROACHING:   { type: 'EARNINGS_APPROACHING',   urgency: 'HIGH',   message: 'reports earnings soon',            action: 'Review your position before the report' },
    DIVIDEND_APPROACHING:   { type: 'DIVIDEND_APPROACHING',   urgency: 'MEDIUM', message: 'has a dividend ex-date approaching', action: 'Ensure you hold through ex-date to qualify' },
    ANALYST_RATING_CHANGE:  { type: 'ANALYST_RATING_CHANGE',  urgency: 'MEDIUM', message: 'analyst rating has changed',        action: 'Review the updated analyst consensus' },
    AI_SIGNAL_CHANGED:      { type: 'AI_SIGNAL_CHANGED',      urgency: 'LOW',    message: 'AI suggested zones updated',        action: 'Review the updated trade plan' },
  }
  ...
  for (const log of recentAlertLogs) {
    ...
    if (position) {
      ...
      if (priceData) {
        const pnlPct = ((priceData.price - position.avgEntryPrice) / position.avgEntryPrice * 100).toFixed(1)
  ...
  const urgencyOrder: Record<TriggerUrgency, number> = { HIGH: 0, MEDIUM: 1, LOW: 2 }
  triggers.sort((a, b) => urgencyOrder[a.urgency] - urgencyOrder[b.urgency])

  const unique = triggers.filter((t, i) =>
    triggers.findIndex(x => x.type === t.type && x.symbol === t.symbol) === i
  )
...
export const buildSectorHeatmap = async (userId: string) => {
  ...
  if (cached) {
    ...
    for (const [sectorName, ticker] of sectorEntries) {
      try {
        ...
        console.error(`Failed to fetch ETF ${ticker} for sector ${sectorName}`);
      }
    }

    cachedAt = new Date().toISOString();
    await setCache(DASHBOARD_SECTOR_CACHE_KEY, { rawPerformance, cachedAt }, DASHBOARD_SECTOR_CACHE_TTL);
  }

  // 2. Calculate User Exposure (Stays the same)
  const portfolios = await prisma.portfolio.findMany({
    where:   { userId },
    include: { positions: { select: { symbol: true, sector: true, quantity: true, avgEntryPrice: true } } },
  });

  const positions = portfolios.flatMap(p => p.positions);
  const totalValue = positions.reduce((sum, p) => sum + p.quantity * p.avgEntryPrice, 0);
  ...
  const sectors: SectorHeatmapItem[] = Object.entries(SECTOR_ETF_MAP).map(([sectorName]) => {
    ...
    const userExposurePct = totalValue > 0 && exposure
    ...
    } else if (userExposurePct > 25 && perf1d < 0) { // Using 25% as threshold
      signal = 'OVEREXPOSED';
    } else if (perf1d > 0) {
    ...
    return Promise.all(
      top.map(async (stock) => {
        try {
          ...
          console.error(`Failed to fetch sparkline for ${stock.symbol}:`, e)
          return { ...stock, sparkline: [] }
        }
      }),
    )
  } catch (err) {
    console.error('Failed to build trending stocks:', err)
    return []
  }
}
...
export const getDashboard = async (
  ...
  const [briefing, portfolio, impactNews, smartTriggers, sectorHeatmap, trendingStocks] = await Promise.all([
  ...
  return { briefing, portfolio, impactNews, smartTriggers, sectorHeatmap, trendingStocks }
}
