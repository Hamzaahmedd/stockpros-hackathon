...
export const createRoleValidator = z.object({
  name: z.string().min(2, { message: "Role name is required and cannot be empty." })
  .regex(ALL_CAPS_SNAKE_CASE_REGEX, {
      message: "Role name must be in all caps snake case (e.g., 'SALES_MANAGER' or 'HR_ADMIN_V2')."
    }),
  description: z.string().min(5, { message: "Role description is required and must be at least 5 characters long." }),
});
...
export const assignRoleValidator = z.object({
  roleIds: z.array(z.string().uuid()).min(1, { message: "At least one role ID must be provided." }),
