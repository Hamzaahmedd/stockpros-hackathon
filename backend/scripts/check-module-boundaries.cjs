...
for (const folder of legacyFolders) {
  if (fs.existsSync(path.join(src, folder)))
    violations.push(`legacy layer folder still exists: src/${folder}`)
}
...
for (const file of files) {
  ...
  for (const match of imports) {
    ...
    if (
      ...
      sourceParts[0] === 'modules' &&
      targetParts[0] === 'modules' &&
      sourceParts[1] !== targetParts[1] &&
      !['public.ts', 'index.ts'].includes(path.basename(target))
    ) {
      violations.push(
        `private cross-module import: ${path.relative(root, file)} -> ${path.relative(root, target)}`,
      )
    }
  }
}
...
console.log(
  `Module boundaries valid; ${files.length} TypeScript files checked.`,
)
