...
try {
  if (!fs.existsSync(schemaPath)) {
    console.error(
      `Error: Could not find schema.prisma at expected location: ${schemaPath}`,
    )
    console.error(
      "Please ensure the script is located one directory level above the 'prisma' folder (e.g., inside 'scripts/').",
    )
    process.exit(1)
  }
    ...
    if (
      ...
      if (!isRelationField) {
        ...
        if (fieldName && fieldName !== mapped) {
          ...
          const newLine =
            `${fieldName} ${fieldType} ${rest} @map("${mapped}")`.trim()
